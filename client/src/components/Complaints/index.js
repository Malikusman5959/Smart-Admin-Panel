...
function Complaints({ tab }) {
  ...
  const createAndDownloadPdf = async () => {
    ...
    await axios
      .post(`${process.env.REACT_APP_API}/create-pdf-complaint`, {
        templet: reportData,
        startDate: moment(reportStartDate).format('L'),
        endDate: moment(reportEndDate).format('L'),
        complaintInfo: reportDataInfo,
      })
      .then(() =>
  ...
  const getDate = async () => {
    const url = `${process.env.REACT_APP_API}/v1/complaintDate?startDate=${moment(reportStartDate).format('L')}&endDate=${moment(reportEndDate).format('L')}`;
    const url1 = `${process.env.REACT_APP_API}/v1/complaintDateInfo?startDate=${moment(reportStartDate).format('L')}&endDate=${moment(reportEndDate).format('L')}`;
            ...
            >
              {
                ...
                <div className="noScroll" style={{ width: '400px', height: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center' }}>
                  ...
                  <p style={{ marginTop: '-8px', color: '#2575c0', fontSize: '27px', fontWeight: '600', letterSpacing: '2px' }}> Complaints Report </p>
                    ...
                    if (
                      ...
                      return (
                        list ?
                          ...
                          <ComplaintCardSquare setselected={setselected} item={item} />
                      );
                    }
                  }
                } else {
